...
- 「向こう5年で結婚する確率」は 2015→2020 国勢調査のコホート追跡。式は `(2015年 cur 歳未婚率 − 2020年 cur+5 歳未婚率) / 2015年 cur 歳未婚率`。2015年データは `src/data/population-2015.json`（MHLW 平成令和元年版厚生労働白書 図表1-1-8、出典：総務省国勢調査）。断面法と比べ中高年の過大評価を解消（40-44歳男: 断面 ~10% → コホート ~0%）。15-19歳の2015値は国勢調査の一般値 0.997 で代用
- `population.json` の総人口合計が公式126.15M に対し **約120M（約5%不足）**。70-74/75-79歳と80+バケットに見積もり不足が残る。50歳時未婚率は公式28.3%/17.8% に整合済だが、高齢層を母数に含むファネル計算では母数過小。具体値は e-Stat 不詳補完CSV (statdisp_id=0004006706) をブラウザで手動DLして再入力する必要あり
- `income-education.json` の 1,872セルは公式集計 (表04000) の秘匿セル多数のため、**ログノーマル近似** (中央値・ピーク帯が就調2022公表集計に整合) で再生成した推定値。`data/scripts/regen_income_edu.py` で再生成可能
- `occupation.json` の年齢別セルも同様に、公式全体値 (男事務0.155/女事務0.283等) と既知の年齢傾斜から**傾斜推定**した近似値
